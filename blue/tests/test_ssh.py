...
that absence of `<provider>-ssh-keys` selects keygen on both providers, that a
build renders the placeholder path and never names `$HOME`, that opt-out
passes through untouched, and that the create matrix, the preflight and the
cleanup reach ONCE with this package's fixtures. Every test redirects `~/.ssh`
into a temporary home: nothing here may touch the real one.
"""
...
def test_build_renders_a_stable_placeholder_path(home):
    ...
    opts = ssh.with_machine_key({**keygen_vultr(), "blue/event": "build"})
...
def test_a_dry_run_renders_the_placeholder_too(home):
    opts = ssh.with_machine_key({**keygen_vultr(), "blue/event": "create", "blue/dry-run": True})
    assert opts["ssh-public-key-path"].startswith(ssh.build_placeholder_dir)


def test_real_events_render_the_real_path(home):
    opts = ssh.with_machine_key({**keygen_vultr(), "blue/event": "create"})
    assert opts["ssh-private-key-path"] == str(home / ".ssh" / "rybbit-vultr-keygen-fixture")
    assert opts["ssh-public-key-path"] == str(home / ".ssh" / "rybbit-vultr-keygen-fixture.pub")


def test_opt_out_passes_through_untouched(home):
    for event in ["build", "create", "delete"]:
        vultr = ssh.with_machine_key({**vultr_fixture(), "blue/event": event})
        assert vultr["vultr-ssh-keys"] == "faa53dae-f289-4bba-bf90-8997131ca40a"
        assert vultr.get("ssh-public-key-path") is None, event
        assert vultr.get("ssh-keygen") is None, event
        digitalocean = ssh.with_machine_key({**fixture(), "blue/event": event})
        assert digitalocean["digitalocean-ssh-keys"] == "58495393"
        assert digitalocean.get("ssh-keygen") is None, event
...
def test_identity_args_select_the_generated_key_only_in_keygen_mode(home):
    ...
    assert ssh.identity_args(ssh.with_machine_key({**vultr_fixture(), "blue/event": "create"})) == []
...
async def test_first_create_generates_the_keypair(home):
    opts = await ssh.ensure_key({**keygen_vultr(), "blue/event": "create"}, none_state)
    prv = home / ".ssh" / "rybbit-vultr-keygen-fixture"
    pub = home / ".ssh" / "rybbit-vultr-keygen-fixture.pub"
    ...
    assert "rybbit-vultr-keygen-fixture managed by Colors" in pub.read_text()
...
async def test_a_key_without_state_is_never_overwritten(home):
    prv = home / ".ssh" / "rybbit-keygen-fixture"
    write(prv, "irreplaceable")
    write(home / ".ssh" / "rybbit-keygen-fixture.pub", "ssh-ed25519 AAAA test")
...
async def test_opt_out_generates_nothing(home):
    for opts in [vultr_fixture(), fixture()]:
        result = await ssh.ensure_key({**opts, "blue/event": "create"}, none_state)
...
def test_preflight_lists_keys_with_the_selected_providers_token(home):
    ...
    ssh.preflight(ssh.with_machine_key({**keygen_vultr(), "blue/event": "create",
...
def test_preflight_refuses_a_foreign_key_and_says_do_not_delete_it(home):
    write(home / ".ssh" / "rybbit-vultr-keygen-fixture.pub", "ssh-ed25519 OURS comment")
    opts = ssh.preflight(
        ssh.with_machine_key({**keygen_vultr(), "blue/event": "create"}),
        lambda _p, _t: [{"id": "abc", "name": "rybbit-vultr-keygen-fixture",
...
def test_preflight_is_skipped_in_opt_out_mode(home):
    ...
    opts = ssh.preflight({**vultr_fixture(), "blue/event": "create"}, boom)
...
def test_delete_removes_the_keypair(home):
    write(home / ".ssh" / "rybbit-keygen-fixture", "private")
    write(home / ".ssh" / "rybbit-keygen-fixture.pub", "public")
    ssh.cleanup_step({**keygen(), "blue/event": "delete", "ssh-keygen": True})
    assert not (home / ".ssh" / "rybbit-keygen-fixture").exists()
    assert not (home / ".ssh" / "rybbit-keygen-fixture.pub").exists()
...
def test_cleanup_is_inert_on_create_and_in_opt_out_mode(home):
    ...
    assert (home / ".ssh" / "rybbit-keygen-fixture").exists()
