...
def test_alias_is_the_profile():
    assert ssh_config.host_alias(fixture()) == "rybbit-fixture"
...
def test_the_marker_is_the_alias_alone():
    ...
    assert ssh_config.begin_marker("rybbit-vultr") == "# BEGIN rybbit-vultr ANSIBLE MANAGED BLOCK"
    assert ssh_config.end_marker("rybbit-vultr") == "# END rybbit-vultr ANSIBLE MANAGED BLOCK"
...
def test_owned_markers_hold_the_one_marker():
    ...
    assert ssh_config.owned_markers("rybbit-vultr") == {
        "begin": {"# BEGIN rybbit-vultr ANSIBLE MANAGED BLOCK"},
...
def test_host_patterns_are_read_from_a_host_line():
    assert ssh_config.host_patterns("Host rybbit-fixture") == ["rybbit-fixture"]
    assert ssh_config.host_patterns("  host   web rybbit-fixture  db ") == ["web", "rybbit-fixture", "db"]
    assert ssh_config.host_patterns("    HostName 192.0.2.1") is None
    assert ssh_config.host_patterns("Match host rybbit-fixture") is None


def test_a_foreign_stanza_is_found():
    lines = ["Host other", "    HostName 192.0.2.1", "", "Host rybbit-fixture"]
    assert ssh_config.foreign_stanza_line(lines, "rybbit-fixture") == 4


def test_our_own_block_is_not_foreign():
    alias = "rybbit-fixture"
...
def test_a_block_under_a_package_prefixed_marker_is_foreign():
    ...
    alias = "rybbit-vultr"
    lines = [f"# BEGIN rybbit {alias} ANSIBLE MANAGED BLOCK",
             f"Host {alias}",
             f"# END rybbit {alias} ANSIBLE MANAGED BLOCK"]
    assert ssh_config.foreign_stanza_line(lines, alias) == 2


def test_a_multi_pattern_host_line_counts():
    assert ssh_config.foreign_stanza_line(["Host web rybbit-fixture db"], "rybbit-fixture") == 1


def test_an_unrelated_file_is_left_alone():
    assert ssh_config.foreign_stanza_line(["Host build", "Host rybbit-other"],
                                          "rybbit-fixture") is None
...
def test_adopt_error_names_the_file_and_the_line(home):
    ...
    assert "`Host rybbit-fixture` at line 4" in error
...
def test_adopt_error_passes_our_own_block_and_a_missing_file(home):
    ...
    write_config(home, f"{ssh_config.begin_marker('rybbit-fixture')}\n"
                       "Host rybbit-fixture\n    HostName 192.0.2.1\n"
                       f"{ssh_config.end_marker('rybbit-fixture')}\n")
    assert ssh_config.adopt_error(fixture()) is None
...
def test_preflight_reads_the_redirected_file(home):
    ...
    write_config(home, "Host rybbit-fixture\n    HostName 192.0.2.1\n")
...
def test_the_local_stage_renders_three_files():
    ...
    assert all("rybbit-ansible-local" in t for t in targets)
...
def test_the_rendered_play_carries_the_identity_pair_only_in_keygen_mode():
    ...
    assert "IdentityFile ~/.ssh/rybbit-keygen-fixture" in keygen_play
