...
async def start_step(original, env=None):
    async def after(opts, environment, ctx):
        if ctx['real'] and ctx['event'] == 'delete':
            ...
            return {**result, "ip": opts["ip"]} if not failed(result) and opts.get("ip") and not result.get("rybbit/already-destroyed") else result
...
def create_workflow():
    wf = workflow(start="rybbit/start", wire_fn=wire_fn, next_fn=lambda _step, successors, opts: [] if opts.get("rybbit/already-destroyed") or failed(opts) else [(step, opts) for step in (successors or [])])
    wf = advice_add(wf, "rybbit/dns", "before", "rybbit.workflow/backend",
