...
def requirements(opts):
    ...
    return {'single_host': True, 'legacy_state_keys': [opts['profile'] + '/rybbit-infrastructure.tfstate'],
            'security': {'ingress': [{'id': 'ssh', 'protocol': 'tcp', 'from_port': 22, 'to_port': 22, 'sources': ssh}]
                        + [{'id': 'http-' + str(port), 'protocol': 'tcp', 'from_port': port, 'to_port': port, 'sources': http} for port in (80,443) if http] + ([{'id':'http3','protocol':'udp','from_port':443,'to_port':443,'sources':http}] if http else []), 'egress': 'all', 'private_filter': False}}
...
def attach(opts, result):
    ...
    if result['status'] == 'destroyed':
        return {**opts, 'blue/exit': 0, 'rybbit/already-destroyed': True}
    params = node(result)
