...
export function computeName(opts: Opts): string {
  const name = String(opts[computeKey(opts, "name")] ?? "");
  return name.length > 0 ? name : String(opts.profile);
}
...
export async function psql(ip: unknown, query: string): Promise<string | undefined> {
  const out = String(await sshOut(ip, stackEnv +
...
export async function eventCount(ip: unknown): Promise<number | undefined> {
  const out = await clickhouse(ip, "SELECT count() FROM $t");
...
export async function acceptanceStep(opts: Opts): Promise<Opts> {
  ...
  const verdict = site === undefined
    ...
    : ingestionVerdict(await sendEvent(base, site),
                       before, await waitIngested(ip, before as number, 10));
