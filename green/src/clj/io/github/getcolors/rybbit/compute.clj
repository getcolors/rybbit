...
(defn requirements [opts]
  (let [ssh (request/source-cidrs opts "ssh-sources" "rybbit-ssh-sources") http (request/source-cidrs opts "http-sources" "rybbit-http-sources")]
    {:single_host true :legacy_state_keys [(str (:profile opts) "/rybbit-infrastructure.tfstate")]
     :security {:ingress (into [{:id "ssh" :protocol "tcp" :from_port 22 :to_port 22 :sources ssh}]
                              (when (seq http) (concat (for [port [80 443]] {:id (str "http-" port) :protocol "tcp" :from_port port :to_port port :sources http}) [{:id "http3" :protocol "udp" :from_port 443 :to_port 443 :sources http}]))) :egress "all" :private_filter false}}))
...
(defn attach [opts result]
  (cond
    ...
    (= "destroyed" (:status result)) (assoc opts :green/exit 0 :rybbit/already-destroyed true)
