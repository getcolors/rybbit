(ns io.github.getcolors.rybbit.ssh-config-test
...
(defn- with-config
  ...
  [content f]
  (let [home (str (fs/create-temp-dir {:prefix "rybbit-ssh-config"}))
...
(deftest the-marker-is-the-alias-alone
  ...
  ;; too would repeat it: "# BEGIN rybbit rybbit-vultr".
  (is (= "# BEGIN rybbit-vultr ANSIBLE MANAGED BLOCK"
         (ssh-config/begin-marker "rybbit-vultr")))
  (is (= "# END rybbit-vultr ANSIBLE MANAGED BLOCK"
         (ssh-config/end-marker "rybbit-vultr"))))
...
(deftest owned-markers-hold-the-one-marker
  ...
  (is (= {:begin #{"# BEGIN rybbit-vultr ANSIBLE MANAGED BLOCK"}
          :end #{"# END rybbit-vultr ANSIBLE MANAGED BLOCK"}}
         (ssh-config/owned-markers "rybbit-vultr"))))

;; §5 never adopt

(deftest host-patterns-are-read-from-a-host-line
  (is (= ["rybbit-fixture"] (ssh-config/host-patterns "Host rybbit-fixture")))
  (is (= ["web" "rybbit-fixture" "db"] (ssh-config/host-patterns "  host   web rybbit-fixture  db ")))
  (is (nil? (ssh-config/host-patterns "    HostName 192.0.2.1")))
  (is (nil? (ssh-config/host-patterns "Match host rybbit-fixture"))))

(deftest a-foreign-stanza-is-found
  (let [lines ["Host other" "    HostName 192.0.2.1" "" "Host rybbit-fixture"]]
    (is (= 4 (ssh-config/foreign-stanza-line lines "rybbit-fixture")))))

(deftest our-own-block-is-not-foreign
  (let [alias "rybbit-fixture"
...
(deftest a-block-under-a-package-prefixed-marker-is-foreign
  ...
  (let [alias "rybbit-vultr"
        lines [(str "# BEGIN rybbit " alias " ANSIBLE MANAGED BLOCK")
               (str "Host " alias)
               (str "# END rybbit " alias " ANSIBLE MANAGED BLOCK")]]
    (is (= 2 (ssh-config/foreign-stanza-line lines alias)))))

(deftest a-multi-pattern-host-line-counts
  (is (= 1 (ssh-config/foreign-stanza-line ["Host web rybbit-fixture db"]
                                           "rybbit-fixture"))))

(deftest an-unrelated-file-is-left-alone
  (is (nil? (ssh-config/foreign-stanza-line ["Host build" "Host rybbit-other"]
                                            "rybbit-fixture"))))
  ...
  (with-config "Host other\n    HostName 192.0.2.1\n\nHost rybbit-fixture\n    User root\n"
    (fn [file]
      (let [err (ssh-config/adopt-error (fixture))]
        ...
        (is (str/includes? err "`Host rybbit-fixture` at line 4"))
        (is (str/includes? err "will not overwrite it"))))))

(deftest adopt-error-passes-our-own-block-and-a-missing-file
  (with-config (str (ssh-config/begin-marker "rybbit-fixture") "\n"
                    "Host rybbit-fixture\n    HostName 192.0.2.1\n"
                    (ssh-config/end-marker "rybbit-fixture") "\n")
...
(deftest preflight-reads-the-redirected-file
  ...
  (with-config "Host rybbit-fixture\n    HostName 192.0.2.1\n"
    (fn [_]
      (let [r (ssh-config/preflight! (fixture))]
        (is (= 1 (:green/exit r)))
        (is (str/includes? (:green/err r) "already declares")))))
...
(deftest the-local-stage-renders-three-files
  (let [targets (map #(str (:target %)) (tools/ansible-local-specs (fixture)))]
    ...
    (is (every? #(str/includes? % "rybbit-ansible-local") targets))))
...
(deftest the-rendered-play-carries-the-identity-pair-only-in-keygen-mode
  (let [keygen-play (render-play (keygen))
    ...
    (is (str/includes? keygen-play "IdentityFile ~/.ssh/rybbit-keygen-fixture"))
