(ns io.github.getcolors.rybbit.ssh-test
  ...
  that absence of `<provider>-ssh-keys` selects keygen on both providers, that
  a build renders the placeholder path and never names `$HOME`, that opt-out
  passes through untouched, and that the create matrix, the preflight and the
  cleanup reach ONCE with this package's fixtures. Every test redirects
  `~/.ssh` into a temporary home: nothing here may touch the real one."
...
(defn- with-home
  ...
  [f]
  (let [home (str (fs/create-temp-dir {:prefix "rybbit-ssh"}))]
    (try
...
(deftest build-renders-a-stable-placeholder-path
  ...
  (with-home
    (fn [_]
      (let [opts (ssh/with-machine-key (assoc (keygen-vultr) :green/event :build))]
...
(deftest a-dry-run-renders-the-placeholder-too
  (with-home
    (fn [_]
      (let [opts (ssh/with-machine-key (assoc (keygen-vultr) :green/event :create :green/dry-run true))]
        (is (str/starts-with? (:ssh-public-key-path opts) ssh/build-placeholder-dir))))))

(deftest real-events-render-the-real-path
  (with-home
    (fn [home]
      (let [opts (ssh/with-machine-key (assoc (keygen-vultr) :green/event :create))]
        (is (= (str (io/file home ".ssh" "rybbit-vultr-keygen-fixture")) (:ssh-private-key-path opts)))
        (is (= (str (io/file home ".ssh" "rybbit-vultr-keygen-fixture.pub")) (:ssh-public-key-path opts)))))))

(deftest opt-out-passes-through-untouched
  (with-home
    (fn [_]
      (doseq [event [:build :create :delete]]
        (let [opts (ssh/with-machine-key (assoc (vultr-fixture) :green/event event))]
          (is (= "faa53dae-f289-4bba-bf90-8997131ca40a" (:vultr-ssh-keys opts)))
...
(deftest identity-args-select-the-generated-key-only-in-keygen-mode
  ...
  (with-home
    (fn [_]
      ...
      (is (= [] (ssh/identity-args (ssh/with-machine-key (assoc (vultr-fixture) :green/event :create))))))))
...
(deftest first-create-generates-the-keypair
  (with-home
    (fn [home]
      (let [opts (ssh/ensure-key! (assoc (keygen-vultr) :green/event :create) (constantly nil))
            prv (io/file home ".ssh" "rybbit-vultr-keygen-fixture")
            pub (io/file home ".ssh" "rybbit-vultr-keygen-fixture.pub")]
        ...
        (testing "ed25519, no passphrase, profile-named comment"
          ...
          (is (str/includes? (slurp pub) "rybbit-vultr-keygen-fixture managed by Colors")))
...
(deftest a-key-without-state-is-never-overwritten
  (with-home
    (fn [home]
      (let [prv (str (io/file home ".ssh" "rybbit-keygen-fixture"))]
...
(deftest opt-out-generates-nothing
  (with-home
    (fn [home]
      (doseq [opts [(vultr-fixture) (fixture)]]
        (let [r (ssh/ensure-key! (assoc opts :green/event :create) (constantly nil))]
    ...
    (fn [_]
      (let [seen (atom [])]
        (with-redefs [once-ssh/fetch-account-keys (fn [provider token] (swap! seen conj [provider token]) [])]
          ...
          (ssh/preflight! (ssh/with-machine-key (assoc (keygen-vultr) :green/event :create
...
(deftest preflight-refuses-a-foreign-key-and-says-do-not-delete-it
  (with-home
    (fn [home]
      (write! (str (io/file home ".ssh" "rybbit-vultr-keygen-fixture.pub")) "ssh-ed25519 OURS comment")
      (with-redefs [once-ssh/fetch-account-keys
                    (fn [_ _] [{:id "abc" :name "rybbit-vultr-keygen-fixture" :public "ssh-ed25519 THEIRS"}])]
        (let [opts (ssh/preflight! (ssh/with-machine-key (assoc (keygen-vultr) :green/event :create)))]
          (is (= 1 (:green/exit opts)))
  ...
  (with-home
    (fn [_]
      (with-redefs [once-ssh/fetch-account-keys (fn [_ _] (throw (ex-info "must not be called" {})))]
        (is (not (contains? (ssh/preflight! (assoc (vultr-fixture) :green/event :create)) :green/err)))))))
...
(deftest delete-removes-the-keypair
  (with-home
    (fn [home]
      (write! (str (io/file home ".ssh" "rybbit-keygen-fixture")) "private")
      (write! (str (io/file home ".ssh" "rybbit-keygen-fixture.pub")) "public")
      (ssh/cleanup-step (assoc (keygen) :green/event :delete :ssh-keygen true))
      (is (not (.exists (io/file home ".ssh" "rybbit-keygen-fixture"))))
      (is (not (.exists (io/file home ".ssh" "rybbit-keygen-fixture.pub"))))
...
(deftest cleanup-is-inert-on-create-and-in-opt-out-mode
  (with-home
    (fn [home]
      ...
      (is (.exists (io/file home ".ssh" "rybbit-keygen-fixture")))
