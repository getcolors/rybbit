...
are defined by the library. Neutral `rybbit-ssh-sources` and
`rybbit-http-sources` are accepted alongside the selected adapter's legacy keys.
...
DNS remains an application stage with its separate `<profile>/rybbit-dns.tfstate`.

The library refuses existing `<profile>/rybbit-infrastructure.tfstate` before
...
Rybbit also requests UDP443 for HTTP/3, under the same HTTP source policy.
...
Use `RYBBIT_LIB_ROOT` for repository development. Canonical `bb pin` in
