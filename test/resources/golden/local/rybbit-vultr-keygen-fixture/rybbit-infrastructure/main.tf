...
resource "vultr_ssh_key" "machine" {
  name    = "rybbit-vultr-keygen-fixture"
  ssh_key = trimspace(file("/home/build-placeholder/.ssh/rybbit-vultr-keygen-fixture.pub"))
}
...
resource "vultr_firewall_group" "rybbit" {
  description = "rybbit-vultr-keygen-fixture"
}
...
resource "vultr_instance" "rybbit" {
  ...
  label = "rybbit-vultr-keygen-fixture"
...
output "params" {
  ...
  value = { provider = "vultr", ip = vultr_instance.rybbit.main_ip, user = "root", sudoer = "root", name = "rybbit-vultr-keygen-fixture", ssh_key_id = vultr_ssh_key.machine.id }
}
