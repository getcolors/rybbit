...
resource "digitalocean_ssh_key" "machine" {
  name       = "rybbit-keygen-fixture"
  public_key = trimspace(file("/home/build-placeholder/.ssh/rybbit-keygen-fixture.pub"))
}
...
resource "digitalocean_firewall" "rybbit" {
  name        = "rybbit-keygen-fixture-firewall"
...
output "params" {
  value = { provider = "digitalocean", ip = digitalocean_droplet.rybbit.ipv4_address, user = "root", sudoer = "root", name = "rybbit-keygen-fixture", ssh_key_id = digitalocean_ssh_key.machine.id }
}
